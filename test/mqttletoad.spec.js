...
describe('mqttletoad', function() {
  ...
  describe('connect()', function(done) {
    ...
    it('should resolve with the wrapped MqttClient once connected', function() {
      return expect(
        promise,
        'when fulfilled',
    ...
    it('should not allow an invalid message to be published', function() {
      return promise.then(client =>
        expect(client.publish('foo/bar', new Date()), 'to be rejected')
      );
    });
  });
  ...
  describe('mqttletoad client', function() {
    ...
    beforeEach(function() {
      return connect(`mqtt://localhost:${listeningPort}`).then(c => {
        client = c;
      });
    });
    ...
    describe('emit()', function() {
      it('should publish with the (string) message and fulfill with the topic', function() {
        return expect(client.emit('foo', 'bar'), 'to be fulfilled with', 'foo');
      });

      it('should fulfill with the (Buffer) message and fulfill with the topic', function() {
        return expect(
          client.emit('foo', Buffer.from([0x00, 0x01, 0x02])),
          'to be fulfilled with',
          'foo'
        );
      });

      it('should throw if non-string, non-Buffer or non-ArrayBuffer value', function() {
    ...
    describe('on()', function() {
      ...
      it('should subscribe to a topic', function(done) {
        ...
        expect(client.on('foo/bar', () => {}), 'to equal', client);
      });

      it('should execute the handler when exact matching topic received', function(
        done
      ) {
        client.once('suback', () => {
      ...
      it('should execute the handler when wildcard (#) topic received', function(
        done
      ) {
        client.once('suback', () => {
      ...
      it('should execute all matching handlers (in order of specificity)', function(
        done
      ) {
        let count = 0;
        let subacks = 0;
    ...
    describe('once()', function() {
      ...
      describe('when multiple listeners on topic', function() {
        it('should unsubscribe only after all messages received once', function(
          done
        ) {
          client.once('suback', () => {
            client.emit('foo/bar', 'baz');
